...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const gallery:Gallery = await prisma.gallery.findFirst({where:{id:Number(id)}});
  if(!gallery) {
    res.json({
  ...
  if(req.method.toUpperCase()==='DELETE'){
    const deleteResult = await prisma.gallery.delete({where:{id:Number(id)}});
    res.status(200).json({
        status:'ok',
        deleteResult
    })
    ...
    const data: { fields: any; files: any } = await new Promise(
      ...
      // update data
      const updateResult = await prisma.gallery.update({where:{id:Number(id)},
        data:{
