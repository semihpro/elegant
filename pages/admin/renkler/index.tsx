...
const Home: NextPage<{colors:Color[]}> = ({ colors }) => {
  ...
  return (
    ...
    <Breadcrumb items={[{text:"Admin", link:"/admin"}, {text:"Renkler"},]}/>
    </div>
    <div className="search_and_add mt-2 mb-2">
      <input type="text" name="search_text" id="search_text" placeholder="Aranacak kelime"/>
      <Link href={"/admin/renkler/ekle"} className="btn btn-green">Ekle <i className="fa fa-plus" aria-hidden="true"></i></Link>
    </div>
    ...
    </li>
    {
      colors.map(item=>{
        return (<li className="table-row" key={item.id}>
          <div className="col col-1" data-label="Color Name">{item.name}</div>
          <div className="col col-4" data-label="Operations"> 
            <Link href={`/admin/renkler/${item.id}`}><i className="fa fa-pencil tooltip mouse-pointer" aria-hidden="true"> <span className="tooltiptext">Renk Düzenle</span></i> </Link>
            <i onClick={()=>handleDelete(item.id)} className="fa fa-trash tooltip mouse-pointer color-red" aria-hidden="true"> <span className="tooltiptext">Renk Sil</span></i>
            </div>
          </li>)
      })}
